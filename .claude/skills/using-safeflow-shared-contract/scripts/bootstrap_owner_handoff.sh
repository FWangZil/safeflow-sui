...
DEFAULT_RECOMMENDED_PREDEPOSIT_ATOMIC="10000000"
...
Options:
  ...
  --recommended-predeposit-atomic <n>
                                    Suggested AgentWallet stablecoin predeposit amount
  --currency-symbol <symbol>        Display symbol for predeposit hint
...
RECOMMENDED_PREDEPOSIT_ATOMIC="$DEFAULT_RECOMMENDED_PREDEPOSIT_ATOMIC"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --recommended-predeposit-mist) RECOMMENDED_PREDEPOSIT_ATOMIC="${2:-}"; shift 2 ;;
        --recommended-predeposit-atomic) RECOMMENDED_PREDEPOSIT_ATOMIC="${2:-}"; shift 2 ;;
...
if ! [[ "$MIN_AGENT_GAS_MIST" =~ ^[0-9]+$ ]] || ! [[ "$RECOMMENDED_PREDEPOSIT_ATOMIC" =~ ^[0-9]+$ ]]; then
    error "amount values must be integers."
    exit 1
fi
...
cat > "$OWNER_HANDOFF_FILE" <<EOF
{
  ...
  "recommendedPredepositAtomic": $RECOMMENDED_PREDEPOSIT_ATOMIC,
...
echo "1) Transfer SUI gas to agent only if the local runner must submit transactions:"
...
echo "   $PORTAL_URL"
echo "   - Suggested guarded wallet predeposit: $RECOMMENDED_PREDEPOSIT_ATOMIC atomic $CURRENCY_SYMBOL"
