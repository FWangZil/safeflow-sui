# SafeFlow (Sui Edition) - SuiOverflow 2026 Hackathon Intro

## 1. 项目一句话简介
基于 Sui native gasless stablecoin 与 Object Model 的**防注入、可追溯** AI 智能体 (OpenClaw) checkout / AgentPay Guard 产品。

## 2. 黑客松期间新增功能 / Redesign 范围

SafeFlow 是一个已有的 Sui 项目，本次 **SuiOverflow 2026** 提交不是旧版 demo 的原样复用，而是在黑客松期间完成的 **well-defined new feature and redesign**。

黑客松期间的明确交付范围是：将早期 `SessionCap` AgentWallet demo 重构为完整的 **Gasless Checkout + AgentPay Guard** 产品层，包括：

- 商户创建 checkout session，Producer API 在 Postgres 中维护 merchant、agent allowance、payment intent 与 sponsor attempt；
- `executionRail=auto`，自动判断简单 allowlisted stablecoin 支付走 Sui native gasless，复杂 AgentPay Guard 支付走 sponsored `SessionCap` 执行；
- sponsor 支付 guarded execution gas，并可在同一稳定币中收取手续费补偿；
- Agent runner 完整执行：poll intent、验签、ACK、上传 Walrus evidence、按 rail 执行、回写结果；
- Web console redesign：operator setup、merchant checkout/status、public checkout、audit trail。

因此，本项目满足 SuiOverflow 2026 黑客松要求：项目必须是 Sui 上的新项目，或者是 established project 但在 hackathon 期间实现了明确的新功能或 redesign。
...
## 4. 我们如何解决的？(The Solution: Dual Gasless Rails)
...
本项目完美横跨并契合 SuiOverflow 2026 的两大 Track：
