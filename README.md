...
![TickPay Sui Banner](./docs/assets/banner.png) *(如果未来有图片可以放在这里)*
...
3. **结合 Walrus 的审计追踪**: 每次 Agent 发起支付，合约都强制要求其提供一个上传至 Walrus（去中心化存储）的 Blob ID，作为其支付推理/意图的不可篡改审计凭证。

## 核心功能与技术栈

- **智能体安全隔离钱包**: Sui Move 实现的 `AgentWallet` 与 `SessionCap` 机制。
- **精确到秒的流支付限制**: Move 合约内基于 Sui Clock 的时间戳流速计算 (`max_spend_per_second`)。
- **可审计的支付意图 (Walrus Integration)**: 链上强制要求携带链下的意图存储凭证。
...
```bash
cd sui/agent_scripts

# 安装依赖
npm install

# 运行 Agent 脚本 (它会自动生成一个本地的 Agent 私钥并打印地址)
npx tsx index.ts
```
记录下控制台打印出的 **Agent Address**。

*(在实际应用中，这里需要将 `PACKAGE_ID` 替换为你刚刚部署的真实 ID，并让 Human Dashboard 给该地址授予 `SessionCap` 后，解除代码中 `agentExecutePayment` 的注释来执行真实交易。)*
...
npm run dev
```
打开浏览器访问 `http://localhost:3000`。连接你的 Sui 钱包，输入上一步生成的 Agent Address，即可模拟为 Agent 发放限额凭证的流程。
